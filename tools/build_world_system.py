...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Build world-grouped system data from String_Simplified.json"
    )
    default_input = os.path.join(
        os.path.dirname(__file__), "..", "resources", "data", "String_Simplified.json"
    )
    default_output = os.path.join(
        os.path.dirname(__file__), "..", "resources", "data", "world_system.json"
    )
    parser.add_argument(
        "--input", default=os.path.normpath(default_input), help="Input JSON path"
    )
    parser.add_argument(
        "--output", default=os.path.normpath(default_output), help="Output JSON path"
    )
