...
class Handler(PacketHandler):
    def _process_equipment_item(self, items, item_tag, instance_id, char_id, is_new_item=True):
        """处理单个装备项"""
        for item_data in items:
            ...
            # 检查物品标签和实例ID是否匹配
            if (item.main_item.item_tag == item_tag and 
                self._get_instance_id(item, item_tag) == instance_id):
                # 设置wearer_id
                wearer_id = char_id if is_new_item else 0
                self._set_wearer_id(item, item_tag, wearer_id)
                
                logger.info(f"{'Setting' if is_new_item else 'Removing'} item instance {instance_id} wearer_id to {wearer_id}")
                
                # 更新装备
                item_id = item.main_item.item_id
                db.up_item_detail(self.session.player_id, item_id, [item.SerializeToString()])
                return item
        return None
    ...
    def handle(self, session, data: bytes, packet_id: int):
        ...
        logger.info(f"Handling equipment update for character {char_id} with weapon instance {weapon_instance_id}")
        ...
        for preset in character.equipment_presets:
            if preset.preset_index == preset_index:
                ...
                for poster_info in preset.posters:
                    old_poster_instance_ids[poster_info.poster_index] = poster_info.poster_id
        ...
        if old_weapon_instance_id:
            old_item = self._process_equipment_item(items, pb.EBagItemTag_Weapon, old_weapon_instance_id, char_id, False)
            if old_item:
                old_items.append(old_item)
        
        # 处理旧防具
        for equip_type, armor_id in old_armor_instance_ids.items():
            old_item = self._process_equipment_item(items, pb.EBagItemTag_Armor, armor_id, char_id, False)
            if old_item:
                old_items.append(old_item)
        
        # 处理旧映像
        for poster_index, poster_id in old_poster_instance_ids.items():
            old_item = self._process_equipment_item(items, pb.EBagItemTag_Poster, poster_id, char_id, False)
            if old_item:
                old_items.append(old_item)
        ...
        if weapon_instance_id:
            new_item = self._process_equipment_item(items, pb.EBagItemTag_Weapon, weapon_instance_id, char_id, True)
            if new_item:
                target_items.append(new_item)
        
        # 处理新防具
        for armor_info in req.equipment_preset.armors:
            new_item = self._process_equipment_item(items, pb.EBagItemTag_Armor, armor_info.armor_id, char_id, True)
            if new_item:
                target_items.append(new_item)
        
        # 处理新映像
        for poster_info in req.equipment_preset.posters:
            new_item = self._process_equipment_item(items, pb.EBagItemTag_Poster, poster_info.poster_id, char_id, True)
            if new_item:
                target_items.append(new_item)
