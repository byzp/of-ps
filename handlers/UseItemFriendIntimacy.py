...
class Handler(PacketHandler):
    def handle(self, session, data: bytes, packet_id: int):
        ...
        db.set_item_detail(session.player_id, item.SerializeToString(), req.item_id, None)
        
        current_intimacy = db.get_friend_info(session.player_id, req.friend_id, "friend_intimacy") or 0
        ...
        session.send(CmdId.UseItemFriendIntimacyRsp, rsp, packet_id) # 2651 2652 使用道具增加好友亲密度