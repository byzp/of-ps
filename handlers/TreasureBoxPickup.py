...
from proto.net_pb2 import (
    TreasureBoxPickupReq,
    TreasureBoxPickupRsp,
    StatusCode,
    TreasureBoxData,
    PackNotice,
    ItemDetail,
...
class Handler(PacketHandler):
    def handle(self, session, data: bytes, packet_id: int):
        req = TreasureBoxPickupReq()
        req.ParseFromString(data)

        rsp = TreasureBoxPickupRsp()
        ...
        if req.pick_index == -1:
            if len(session.temp_pack) >= 30:
                rsp.status = StatusCode.StatusCode_TEMP_PACK_IS_FULL
            ...
            for item in tb.rewards:
                ...
                if tmp_i < 30:  # TODO 动态背包大小限制
                    if item.main_item.item_tag == EBagItemTag.EBagItemTag_Weapon:
                        instance_id = item.main_item.weapon.instance_id
                    if item.main_item.item_tag == EBagItemTag.EBagItemTag_Armor:
                        instance_id = item.main_item.armor.instance_id
                    item.pack_type = ItemDetail.PackType.PackType_Inventory
                    db.set_item_detail(
                        session.player_id, item.SerializeToString(), None, instance_id
                    )
                    item.pack_type = ItemDetail.PackType.PackType_TempPack
